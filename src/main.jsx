...
const SUPPLEMENTS = [
  // MORNING
  { id: "alphagpc", time: "morning", phase: 3, label: "Alpha-GPC", dose: "300mg (partial yolk reduction) or 600mg (larger yolk reduction)", form: "Capsule", status: "pending", source: "protocol",
    why: "Choline is a named HIGH-impact nutrient pathway in your report — you have a genetically increased dietary choline requirement (PEMT GC variant reduces endogenous choline synthesis). Layered on APOE E4's accelerated neural choline use, this is a documented need, not an inference. Alpha-GPC crosses the blood-brain barrier and directly supports acetylcholine synthesis — better CNS delivery than choline bitartrate in your OwnIt stack. SEQUENCING UPDATE: moved ahead of NMN in the start order. This is now covering a real nutritional gap opened by the lipid strategy (reducing egg yolks to manage LDL/Lp(a)), not just a cognitive optimization — the diet change should be covered as it happens, not queued behind an arbitrary supplement order.",
    target: "APOE E4 · Memory & Brain · Mood & Behavior · Choline (diet gap coverage)",
    note: "START FIRST in the cognitive/energy layer. Dose by how far you actually cut yolks: 300mg/day (~120mg choline) covers partial yolk separation; 600mg/day (~240mg choline) covers a larger reduction — one large yolk is roughly 150mg choline, so calibrate to what you're actually removing. COMT GA means you clear neuroactive compounds slowly — watch for headache or overstimulation before increasing from 300 to 600. Does not overlap mechanistically or via CYP with the Fisetin/Quercetin pulse, so it can run alongside that if needed — the two don't cost each other attribution." },
  { id: "nmn", time: "morning", phase: 3, label: "NMN", dose: "250–500mg", form: "Capsule", status: "pending", source: "protocol",
    ...
    note: "Start 14–21 days AFTER Alpha-GPC, not before. Both have subtle early subjective effects (Alpha-GPC: mild headache or cholinergic \"fog-lift\" in week 1–2; NMN: energy/sleep shifts on a similar timeline) — starting together means you can't tell which one is doing what. Take on empty stomach or light meal. Morning timing supports circadian NAD+ rhythm. Keep to 250–500mg given your FOXO3 TT longevity variant. Sourcing confirmed: ProHealth Longevity NMN Pro 500 (Uthever NMN), already on hand." },
  ...
  { id: "glutathione", time: "morning", phase: 2, label: "S-Acetyl Glutathione", dose: "300mg", form: "Capsule", status: "pending", source: "protocol",
    ...
    note: "300mg (ProHealth Longevity, on hand) — above the original 200mg estimate but well within normal commercial range and glutathione's wide safety margin; no concern. One capsule covers the full dose. Add alongside OwnIt — NAC in your stack still contributes to endogenous glutathione. These work together. Phase 2 anchor for your GST-null status. Hold start until glycine has run 3–4 weeks continuously — confirm actual start date before beginning this, not just the date it was marked active in-app." },
...
const PROTOCOL_CATEGORIES = [
  {
    ...
    actions: [
      ...
      },
      {
        ...
        urgency: "SOON",
        urgencyColor: C.orange,
        shortDesc: "Second in the cognitive/energy queue — starts 14–21 days after Alpha-GPC, not before",
        ...
        brands: "ProHealth Longevity NMN Pro 500 (Uthever NMN) — confirmed sourced, on hand. Alternatives: Tru Niagen (NR), Alive by Science NMN.",
        timing: "Morning, on an empty stomach or with a light meal",
        note: "SEQUENCING: start 14–21 days AFTER Alpha-GPC, not before — reversed from the original plan once Alpha-GPC got pulled forward to cover the choline gap from egg yolk reduction. Both compounds have subtle early subjective effects (Alpha-GPC: mild headache or cholinergic \"fog-lift\" in week 1–2; NMN: energy/sleep shifts on a similar timeline) — starting together means you can't tell which one is doing what. Also addresses your energy expenditure, weight management, and vascular health pathways simultaneously.",
      ...
      },
      {
        ...
        urgency: "NOW",
        urgencyColor: C.red,
        shortDesc: "Alpha-GPC elevated to NOW — covers the choline gap opened by reducing egg yolks for the lipid strategy",
        why: "APOE E4 impairs three specific mechanisms: amyloid clearance from brain tissue, cholesterol redistribution within neurons, and microglial (brain immune cell) activation leading to neuroinflammation. Your current stack addresses systemic inflammation broadly but has no compounds specifically targeting these neural mechanisms. These three additions work on the E4-specific pathways and have the strongest evidence base for E4 carriers specifically. PRIORITY UPDATE: Alpha-GPC is no longer just brain support — reducing egg yolks as part of the lipid/Lp(a) strategy removes a major dietary choline source, and your PEMT GC variant means you can't make up the difference endogenously. This makes Alpha-GPC a nutritional gap to cover on the diet's timeline, not a supplement to queue behind everything else.",
        what: "1. Alpha-GPC — dose scales to how far you cut yolks: 300mg/day (~120mg choline) for partial yolk separation, 600mg/day (~240mg choline) for a larger reduction. One large yolk ≈ 150mg choline — calibrate against what you're actually removing. Crosses blood-brain barrier, supports acetylcholine synthesis.\n\n2. Lion's Mane Mushroom Extract 500–1000mg/day (fruiting body, not mycelium) — stimulates NGF (Nerve Growth Factor), promotes neuronal maintenance and plasticity. Strong evidence for memory & brain VERY HIGH + APOE E4.\n\n3. Trans-Resveratrol 250–500mg/day — activates SIRT1, upregulates BDNF, reduces amyloid aggregation, AND activates eNOS for vascular health. One compound addressing both APOE E4 and your vascular pathway simultaneously.",
        brands: "Alpha-GPC: NOW Foods 300mg (confirmed, on hand). Lion's Mane: Force Factor 500mg — confirmed 100% fruiting body, matches spec (on hand). Resveratrol: NOW Foods 200mg trans-resveratrol standardized (confirmed, on hand).",
        ...
        note: "Order: Alpha-GPC FIRST — start now, or alongside the yolk reduction as it happens. It does not overlap mechanistically or via CYP with the Fisetin/Quercetin senolytic pulse, so it can run concurrently with that if timing requires it. Then Resveratrol, then Lion's Mane, one at a time, roughly two weeks apart, per the original 60-day build-out. SEPARATE FROM THIS CARD: NMN starts 14–21 days after Alpha-GPC begins — see the NMN card — because both have early subjective effects (headache/overstimulation for Alpha-GPC, energy/sleep shifts for NMN) that need to be told apart.",
        pathways: ["APOE E4", "Memory & Brain", "Vascular Health", "Mood & Behavior", "Choline (diet gap coverage)"],
      },
      {
        id: "citrulline",
      ...
      {
        id: "spermidine_declined",
        title: "Spermidine — CONSIDERED, DECLINED",
        urgency: "DECLINED",
        urgencyColor: C.muted,
        shortDesc: "Weak human evidence at deliverable doses, no genetic driver, conflicts with senolytic pulse attribution",
        why: "Evaluated as a possible daily longevity addition (surfaced via the IM8 CRT8 blend evaluation in June). Mechanism is genuine — spermidine induces autophagy, a different pathway from senolytic cell clearance — but the human evidence doesn't support it at achievable doses. A 2024 RCT gave healthy older men 40mg/day of high-purity spermidine for 28 days and found no meaningful rise in circulating polyamine levels; a separate cognitive-decline trial at 1.2mg/day found the same. Most commercial products (wheat germ extract type) deliver only 1–6mg. None of your panel markers (GSTM1/GSTT1/GPX1, APOE E4, PEMT, CETP, GDF5/VEGFA/COL12A1, FOXO3) point at autophagy or polyamine metabolism specifically, so unlike Fisetin there's no personalized rationale pulling this in.",
        what: "DECLINED — not added. Reasoning logged so it isn't re-litigated without new information.",
        brands: "N/A",
        timing: "N/A",
        note: "Protocol-fit issue on top of the weak evidence: a continuous daily senomorphic sitting alongside your first senolytic pulse would muddy the clean on/off design the pulse protocol depends on for attribution — same conflict originally flagged with IM8. Revisit only if a human trial demonstrates systemic bioavailability at a commercially available dose.",
        pathways: ["Senescence", "Longevity"],
      },
      {
        id: "berberine_pending_md",
        title: "Berberine — POSSIBLE, PENDING DOCTOR CONVERSATION",
        urgency: "AWAITING MD",
        urgencyColor: C.accentB,
        shortDesc: "Real LDL-lowering mechanism, but CYP3A4 inhibition could interact with a statin if one is prescribed",
        why: "Evaluated alongside spermidine (surfaced via the IM8 CRT8 blend evaluation in June). Activates AMPK and separately upregulates hepatic LDL receptor expression — a genuinely different mechanism from statins, with a reasonably solid human trial base for both glucose and LDL effects, stronger evidence than most items on this list. No interaction found against Foquest (methylphenidate is cleared by hydrolysis, not CYP-dependent) or valacyclovir (renal/transporter clearance, not hepatic CYP). The interaction that matters hasn't happened yet: berberine inhibits CYP3A4, CYP2D6 and CYP2C9 — CYP3A4 is how most statins are cleared. If the lipid strategy conversation (LDL 4.5, Lp(a) 147) results in a statin prescription, self-adding berberine on top raises statin blood levels and myopathy risk.",
        what: "HOLD until the lipid management conversation happens. Typical dose if cleared: 500mg, 2–3x/day with meals.",
        brands: "Not sourced yet — decision pending.",
        timing: "Do not start before the doctor conversation, regardless of outcome.",
        note: "Ask directly at that appointment whether berberine helps or complicates a statin choice — this is the right venue for the decision, not a self-directed add beforehand. Separate caution independent of the doctor's answer: berberine lowers blood glucose, and your insulin sensitivity is already excellent (HOMA-IR ~0.6) with no documented glucose-pathway variant driving a need for it. Combined with fasted Zone 2 cardio on electrolytes only, this is a real hypoglycemia setup with no corresponding gap to close for you. Weigh that against the LDL rationale before deciding, even if the statin question resolves cleanly.",
        pathways: ["Cholesterol", "Vascular Health"],
      },
    ],
  },
  {
